...
"let g:pdv_cfg_Author = "Pablo López Torres <pablolopeztorres@gmail.com>"
...
"        if (getline(l:line) =~ g:pdv_re_func || getline(l:line) =~ g:pdv_re_attribute || getline(l:line) =~ g:pdv_re_class) && getline(l:line - 1) !~ g:pdv_re_comment
        if getline(l:line - 1) !~ g:pdv_re_comment
			let l:docLines = 0
...
func! PhpDoc()
    ...
    if l:currentline < 2 " If it is in the beginning
        let l:result = PhpDocFile()

    elseif l:line =~ g:pdv_re_func
...
"    exe "norm! " . commentline . "G$"
    ...
    exe l:txtBOL . g:pdv_cfg_CommentHead . g:pdv_cfg_EOL
	exe l:txtBOL . g:pdv_cfg_Comment1 . l:filename . " " . g:pdv_cfg_EOL
    exe l:txtBOL . g:pdv_cfg_Commentn . g:pdv_cfg_EOL
