...
function! s:PHPCustomFolds() " {{{
    ...
    " Abstract functions
	call s:PHPFoldProperties('abstract.*function', ";", s:FOLD_WITH_PHPDOC)

    " functions inside an interface
	call s:PHPFoldProperties('function.*;\s*$', ";", s:FOLD_WITH_PHPDOC)
	
	" Fold class properties with PhpDoc (var $foo = NULL;)
	"call s:PHPFoldProperties('^\s*var\s\$', ";", s:FOLD_WITH_PHPDOC, 1, 1)

    " Fold class constants with PhpDoc (const MYOWNCONSTANT = 'something';)
	call s:PHPFoldProperties('^\s*const\s*', ";", s:FOLD_WITH_PHPDOC, 1, 1)

	" Fold class properties with PhpDoc (public $foo = NULL;)
	call s:PHPFoldProperties('^\s*\(abstract \|static \|final \|private \|protected \|public \|var \)\+\s*\$', ";", s:FOLD_WITH_PHPDOC, 1, 1)
	...
	call s:PHPFoldProperties('^\s*\(require\|include\)\s*', ";", s:FOLD_WITH_PHPDOC)
...
function! PHPFoldText() " {{{
	...
	elseif strridx(lineString, "\/\*\*") != -1
		...
		while currentLine < v:foldend
            ...
            if strridx(line, " * @var ") != -1
               let returnVarType = substitute(line, '^\s*\*\s*@var\s*', '', 'g')
               let returnVarType = substitute(returnVarType, '\s.*', '', 'g')
            endif

            if strridx(line, " * @return ") != -1
               let returnVarType = substitute(line, '^\s*\*\s*@return\s*', '', 'g')
               let returnVarType = substitute(returnVarType, '\s.*', '', 'g')
            endif

            if strridx(line, " * @deprecated") != -1
               let deprecated = 1
            endif

            if strridx(line, " * @implement") != -1
               let implement = 1
            endif
	...
	" if currentLine != v:foldend 
        ...
        " Classes
        let lineString = substitute(lineString, '^abstract class ', ':@: ', 'g')
        let lineString = substitute(lineString, '^class ', ':%: ', 'g')
        let lineString = substitute(lineString, '^interface ', ':i: ', 'g')
        let lineString = substitute(lineString, ' extends ', ' : ', 'g')
        let lineString = substitute(lineString, ' implements ', ' < ', 'g')
		...
		" Detect access
		if strridx(lineString, 'public ') != -1
			let access = '+'
		elseif strridx(lineString, 'protected ') != -1
			let access = '#'
		elseif strridx(lineString, 'private ') != -1
			let access = '-'
		else
			let access = ''
		endif
		...
		" Detect abstract
		if strridx(lineString, 'abstract ') != -1
			let isAbstract = 1
			let access = '@'
		else
			let isAbstract = 0
		...
		" Detect static
		if strridx(lineString, 'static ') != -1
			let isStatic = 1
			let tmpChar = startChar
		...
		" Do the replace
		let lineString = substitute(lineString, '^\(abstract \|static \|final \|public \|protected \|private \)\+', startChar . access . endChar . ' ', 'g')

		" Const
		" Do the replace
		let lineString = substitute(lineString, '^const ', '<-> ', 'g')
        ...
        " Functions
        if strridx(lineString, 'function ') != -1
            let lineString = substitute(lineString, 'function ', '', 'g')
        endif

        if returnVarType != ""
            let lineString = lineString . " : " . returnVarType
        endif

        " Include related
        let lineString = substitute(lineString, '^\(include\|require\|include_once\|require_once\) ',       '   <=(( ', 'g')
    ...
    " Additional space
    let lineString = prefix . lineString . "  "
